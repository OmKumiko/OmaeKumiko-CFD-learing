...
%全亚声速等熵喷管流动
P=ones(1,Nx)-0.023.*x;
T=ones(1,Nx)-0.009333.*x;
u=0.05.*ones(1,Nx)+0.11.*x;
A=zeros(1,Nx);
%定义喷管
for k=1:Nx
    if x(1,k)<=1.5
        A(1,k)=1+2.2*((x(1,k)-1.5)^2);
    else
        A(1,k)=1+0.2223*((x(1,k)-1.5)^2);
    end 
end
%定义偏导数以及预估步修正步矩阵
...
for j=1:5000%时间
    gama=1.4;
    %确定步长
    delta_t=0.5*dx./(u(1,2:Nx-1)+sqrt(T(1,2:Nx-1)));
    dt=min(delta_t);
...
%显示结果
plot(x,p,'Linewidth',1.2,'Color','r'); hold on;
disp(p);
legend("pressure");
xlabel("X");
ylabel("Pressure");
